...
export function validateProgressionSystem(x:any){if(!x||typeof x!=='object'||typeof x.name!=='string'||!x.name.trim()||typeof x.unit!=='string'||!x.unit.trim()||typeof x.description!=='string'||!x.description.trim()||!Array.isArray(x.tiers)||x.tiers.length<3)throw new Error('AI 返回的成长体系 progressionSystem 缺失或格式错误');let last=-1;const tiers=x.tiers.map((t:any,i:number)=>{if(!t||typeof t.name!=='string'||!t.name.trim()||typeof t.description!=='string'||!t.description.trim()||!Number.isFinite(t.requiredProgress)||!Number.isFinite(t.lifespanYears)||t.requiredProgress<0||t.lifespanYears<=0||t.requiredProgress<=last||(i===0&&t.requiredProgress!==0))throw new Error(`成长体系第 ${i+1} 层无效：进度须从0严格递增，寿命须为正数`);last=t.requiredProgress;return {name:t.name.trim(),requiredProgress:Math.round(t.requiredProgress),lifespanYears:Math.round(t.lifespanYears),description:t.description.trim()}});return {name:x.name.trim(),unit:x.unit.trim(),description:x.description.trim(),tiers}}
...
export class RemoteAiProvider implements AiProvider{
 ...
 async generateLifeEvent(c:LifeEventGenerationContext):Promise<PendingLifeEvent>{
  ...
  const text=await this.chat('你是人生事件导演。只返回严格合法JSON对象，不得解释。效果只能使用指定白名单字段，不得输出对象路径。',`本次时间跨越为 ${c.advanceMonths} 个月。生成一个能代表这整个时期最关键变化、适龄且不重复近期内容的事件；不要把它假装成只发生在某一天。阶段重点：${stageGuide}。返回字段 category,title,description,participants,choices；choices严格2-4项，每项含id,label,intent,effects。effects白名单：modify_attribute(attribute为8项之一,amount -10..10), modify_vital(vital为health/energy/mood/stress,amount -20..20), modify_money(account personal/family,amount -100000..100000), modify_progression(amount -20..100), modify_relationship(memberId,amount -20..20), change_location(location), add_character(character含name/role等), update_character(memberId及允许字段), kill_character(memberId,reason), kill_player(reason), add_fact(fact)。每个选择给出合理建议效果；跨越较长时段可建议合理的修炼/成长进度，修仙、武侠、魔法尤其相关；普通生活事件不可无理由突破，进度量应结合相邻层跨度。不得创造任意状态路径。上下文：${JSON.stringify(brief)}`);
