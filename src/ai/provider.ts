...
export class RemoteAiProvider implements AiProvider{
 ...
 private async chat(system:string,user:string,maxTokens=this.settings.maxTokens){
  const s=this.settings; const headers:Record<string,string>={'Content-Type':'application/json'}; let body:any;
  if(s.format==='anthropic'){headers['x-api-key']=s.apiKey;headers['anthropic-version']='2023-06-01';body={model:s.model,max_tokens:maxTokens,temperature:s.temperature,system,messages:[{role:'user',content:user}]}}
  else{headers.Authorization=`Bearer ${s.apiKey}`;body={model:s.model,max_tokens:maxTokens,temperature:s.temperature,messages:[{role:'system',content:system},{role:'user',content:user}]}}
  let response:Response;try{response=await fetch(s.endpoint,{method:'POST',headers,body:JSON.stringify(body)})}catch(e){throw new Error(`网络请求失败：${e instanceof Error?e.message:String(e)}`)}
 ...
 async testConnection(){const result=await this.chat('你是连接测试助手。','只回复“连接成功”。',32);return result.trim()}
