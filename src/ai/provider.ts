...
export class RemoteAiProvider implements AiProvider{
 ...
 async generateWorld(req:WorldGenerationRequest):Promise<GeneratedWorld>{
  ...
  const text=await this.chat('你是严谨而富有原创性的世界构建器。仅返回合法 JSON 对象，所有字段必须完整。不要照抄模板示例，要根据玩家选择重新推演一个具体世界。',`根据世界边界和玩家主题，推演一个全新的、内部自洽且适合模拟完整人生的世界。模板只是类型边界，不是现成剧情；不得读取或复用任何固定家庭、天赋或事件答案。\n必须包含字符串字段 name,eraBackground,socialStructure,coreConflict,livingEnvironment,growthPaths,loveMarriageRules,familyStructure,dangers,lifespan,overview，至少4项的非空字符串数组 birthRegions，以及 progressionSystem={name,unit,description,lifespanMode,tiers}；lifespanMode严格为increasing/decreasing/mixed/unchanged之一；tiers严格3-12层，每层含name,requiredProgress,lifespan:{years,ageless,naturalDeathEnabled},description,advancementConditions字符串数组,risks字符串数组。requiredProgress首层为0且严格递增，years为1-100000；increasing寿命非递减，decreasing非递增，unchanged全部相同，mixed不限；ageless=true时naturalDeathEnabled必须false。现代/古代约80-150年；武侠普通数十年、顶尖约120-200年且不可千年；修仙从凡人约100-150年逐步到数百/数千年；魔法依魔力阶位/种族可数百年；赛博依义体医疗，太空依基因医疗。层名和具体数值由你按世界原创。\n推演时必须说明所选主题如何互相影响，而不是逐项罗列。世界名称、社会制度、地区和矛盾应具体原创。\n本次创作扰动码：${creativeNonce}\n世界边界：${JSON.stringify(templateBrief)}\n玩家选择主题：${JSON.stringify(req.selectedThemes)}`);const x=extractJson(text) as any;const fields=['name','eraBackground','socialStructure','coreConflict','livingEnvironment','growthPaths','loveMarriageRules','familyStructure','dangers','lifespan','overview'];if(!x||typeof x!=='object'||fields.some(k=>typeof x[k]!=='string'||!x[k].trim())||!Array.isArray(x.birthRegions)||!x.birthRegions.length||x.birthRegions.some((v:any)=>typeof v!=='string'||!v.trim()))throw new Error('AI 返回的世界结构字段缺失或格式错误');return {templateId:req.template.id,selectedThemes:req.selectedThemes,generatedAt:new Date().toISOString(),...Object.fromEntries(fields.map(k=>[k,x[k].trim()])),birthRegions:x.birthRegions.map((v:string)=>v.trim()),progressionSystem:validateProgressionSystem(x.progressionSystem)} as GeneratedWorld}
